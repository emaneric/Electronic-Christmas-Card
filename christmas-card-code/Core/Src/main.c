...
static uint8_t test_image2[8] = {
    0b10000000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
};

static uint8_t test_image3[8] = {
    0xFF, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
};

static uint8_t test_image4[8] = {
...
int main(void)
{
  ...
  /*
   * This macro calculates the vdda voltage (as a uint32_t representing the
   * voltage in milliVolts) using the vref internal raw adc value, and the
   * internal calibration value in ROM
   */
  ...
  /*
  Timer 3 configured to trigger every 260μs
  It takes 260 * 64 = 16.64ms to update a single frame
...
//void HAL_DMA_XFER_ERROR_CB_ID


// Take an array of pixel values in the form uint8_t pixel_values[8]
// convert it to two arrays of data to be sent to the BSRR registers the form
// uint8_t data[64]
void generate_BSRR_array(uint8_t pixel_values[8],
                         volatile uint32_t BSRR_row_array[64],
                         volatile uint32_t BSRR_column_array[64]) {

  // Rows are the LED cathodes on port B.
  ...
  for (uint8_t pixel_index = 0; pixel_index < 64; pixel_index++) {
    ...
    if ((pixel_values[input_row_index] >> (7 - input_column_index)) &
        1) { // reverse the order so the MSB of the pixel array coresponds to
             // column 0

      // Calculate the BSRR registers for row and column in order to set the
      // pixel high Turn off the row pin we want and turn on all other row pins
      ...
      // Calculate the BSRR registers for row and column in order to set the
      // pixel low Turn off both row and column pins
    ...
    // Once every bit in the row is processed, go to the next row and start from
    // bit 0 again
